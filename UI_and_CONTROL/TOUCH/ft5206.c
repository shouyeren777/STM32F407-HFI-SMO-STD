...
#include "ctiic.h"
#include "ft5206.h"
#include "../../BSP/SYSTEM/usart/usart.h"
...
uint8_t ft5206_scan(uint8_t mode)
{
    ...
    uint8_t hw_polled = 0; /* 本帧是否真正读了触摸 IC；未读时不得用 sta==0 误判为松手 */
    ...
    /* 仅在本帧已读硬件时根据 sta 更新松手；否则保持 tp_dev.sta，避免“未轮询帧 sta=0”误清按下 */
    if (hw_polled != 0u && ((sta & 0X1F) == 0))  /* 无触摸点按下 */
    {
