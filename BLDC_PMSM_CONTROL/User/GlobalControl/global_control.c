...
#include "../MotorControl/motor_system.h"
#include "../../../UI_and_CONTROL/HMI/LedControl/led_task.h"
#include "../../../UI_and_CONTROL/HMI/LcdControl/lcd_task.h"
#include "../../../UI_and_CONTROL/HMI/LcdControl/lcd_mid.h"
#include "../../../UI_and_CONTROL/HMI/KeyControl/key_task.h"
#include "../../../UI_and_CONTROL/HMI/UsartControl/usart_task.h"
...
//    HAL_TIM_Encoder_Start(&htim3, TIM_CHANNEL_ALL);         //启动编码器接口 
    ...
    HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_1);            //开启互补通道PWM输出
    HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_2);            //开启互补通道PWM输出
    HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_3);            //开启互补通道PWM输出

    HAL_TIM_Base_Start_IT(&htim1);                          //开启定时器中断
    HAL_TIM_Base_Start_IT(&htim2);                          //开启定时器中断
    HAL_TIM_Base_Start(&htim6);
    
    /* Driver enable pins are board-specific on F407; keep disabled here until mapped. */
}
